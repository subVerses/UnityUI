...
public class VisionManager_NPC : MonoBehaviour {
	...
	NPC_Interactable npcChar;
	CharacterInteraction charController;
	SpriteMover charDir;
	...
	void Start () {
		...
		npcChar = npcCollider.GetComponent<NPC_Interactable>();
		charController = GameObject.Find ("CharacterController").GetComponent<CharacterInteraction> ();
		charDir = GameObject.Find ("SpriteController").GetComponent<SpriteMover>();
	...
	{
		if(charController.charMoving)
			return npcChar.inSight(charDir);
		return false;
