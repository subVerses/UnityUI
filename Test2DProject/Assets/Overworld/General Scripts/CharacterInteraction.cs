...
public class CharacterInteraction : MonoBehaviour {
	...
	public const int FRAMESBEFOREMOVE = 10;
	...
	void OnGUI() {
		...
		if(spriteMov.InConversation) {
			...
			if (GUI.RepeatButton (new Rect (0 , Screen.height * 4 / 5, Screen.width / 7, Screen.height / 5), "", skin.FindStyle ("leftButton"))) {
				buttonPushed = true;
			...
			if (GUI.RepeatButton (new Rect (Screen.width * 2 / 7 , Screen.height * 4 / 5, Screen.width / 7, Screen.height / 5), "", skin.FindStyle ("rightButton"))) {
				buttonPushed = true;
				if(spriteMov.getCurrentDirection() == -1) {
	...
	public void SetLastDirection(int ld) {
		lastDirection = ld;
	}
	...
	public void setNPCChat(bool npcCh) {
		npcChat = npcCh;
	}
}
